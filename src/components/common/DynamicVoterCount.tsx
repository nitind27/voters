...
interface DynamicVoterCountProps {
  title: string;
  refreshInterval?: number; // in milliseconds, default 30000 (30 seconds)
}

const DynamicVoterCount: React.FC<DynamicVoterCountProps> = ({ 
  title, 
  ...
  const fetchVoterCount = async () => {
    try {
      setError(null);
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/voterentry`, {
        cache: 'no-store',
        headers: {
...
export default DynamicVoterCount;
